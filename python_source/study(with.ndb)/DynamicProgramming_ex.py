...
# 퀵 정렬과 같은 분할 정보과 다이나믹 프로그래밍을 비교했을 때, 둘 다 최적 부분 구조를 가질 때 사용할 수 있다.
# 차이점은 부분 문제의 중복이다. D.P는 각 부분 문제가 서로 영향을 미치며 부분 문제가 중복되지만 분할 정복은 동일한 부분 문제가 반복 계산 되지 않는다.
