...
# 3번 사람은 1번, 2번 사람이 돈을 뽑을 때까지 기다려야 하기 때문에, 총 3+1+4 = 8분이 필요하게 된다. 4번 사람은 3+1+4+3 = 11분, 5번 사람은 3+1+4+3+2 = 13분이 걸리게 된다. 이 경우에 각 사람이 돈을 인출하는데 필요한 시간의 합은 3+4+8+11+13 = 39분이 된다.
...
for i in people:
    ...
    stack = stack+i
# 차례대로 더할 때, result+=i (x) result+=(result+1) (x) 이런식이면 안 됨. 총 걸린시간이랑 앞사람이 걸린 시간은 다르기 때문.
print(result)
